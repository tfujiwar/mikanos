...
namespace {
  ...
  WithError<size_t> SetupPageMap(PageMapEntry *page_map, int page_map_level, LinearAddress4Level addr, size_t num_4kpages) {
    while (num_4kpages > 0) {
      ...
      // iterate to the next linear address
  ...
  Error LoadELF(Elf64_Ehdr *ehdr) {
    ...
    const auto addr_fisrt = GetFirstLoadAddress(ehdr);
    if (addr_fisrt < 0xffff'8000'0000'0000) {
      return MAKE_ERROR(Error::kInvalidFormat);
    }
...
Vector2D<int> Terminal::CalcCursorPos() const {
  return {ToplevelWindow::kTopLeftMargin + Vector2D<int>{4 + cursor_.x * 8, 5 + cursor_.y * 16}};
}
...
void Terminal::Scroll1() {
  Rectangle<int> move_src{
    ...
    {kColumns * 8, (kRows - 1) * 16}
  };
  window_->Move(ToplevelWindow::kTopLeftMargin + Vector2D<int>{4, 4}, move_src);
  FillRectangle(*window_->InnerWriter(), {4, 4 + cursor_.y * 16}, {kColumns * 8, 16}, ToColor(0));
}
...
Error Terminal::ExecuteFile(const fat::DirectoryEntry &file_entry, char *command, char *first_arg) {
  ...
  const auto addr_fisrt = GetFirstLoadAddress(elf_header);
  if (auto err = CleanPageMaps(LinearAddress4Level{addr_fisrt})) {
    return err;
  }
...
void Terminal::Print(const char c) {
  ...
  if (c == '\n') {
    ...
    WriteAscii(*window_->Writer(), CalcCursorPos(), c, ToColor(0xffffff));
...
Rectangle<int> Terminal::HistoryUpDown(int direction) {
  ...
  Rectangle<int> draw_area{first_pos, {(kColumns - 2) * 8, 16}};
