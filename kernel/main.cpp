...
void InitializeMainWindow() {
  ...
  DrawWindow(*main_window->Writer(), "Main Window");
...
void InitializeTextWindow() {
  ...
  DrawTextbox(*text_window->Writer(), {4, 24}, {win_w - 4 - 4, win_h - 24 - 4});
...
void DrawTextCursor(bool visible) {
  ...
  const auto pos = Vector2D<int>{8 + text_window_index * 8, 24 + 5};
...
void InputTextWindow(char c) {
  ...
  auto pos = []() { return Vector2D<int>{8 + text_window_index * 8, 24 + 6 }; };
...
void TaskB(uint64_t task_id, int64_t data) {
  ...
  while (true) {
    ...
    FillRectangle(*task_b_window->Writer(), {24, 28}, {80, 16}, ToColor(0xc6c6c6));
