...
void InitializeMainWindow() {
  main_window = std::make_shared<ToplevelWindow>(
      160, 52, screen_config.pixel_format, "Main Window");
...
void DrawTextCursor(bool visible) {
  ...
  const auto pos = Vector2D<int>{4 + text_window_index * 8, 5};
...
void InputTextWindow(char c) {
  ...
  auto pos = []() { return Vector2D<int>{4 + text_window_index * 8, 6}; };
...
void TaskB(uint64_t task_id, int64_t data) {
  ...
  while (true) {
    ...
    FillRectangle(*task_b_window->InnerWriter(), {20, 4}, {80, 16}, ToColor(0xc6c6c6));
...
extern "C" void KernelMainNewStack(
  ...
  while (1) {
    ...
    FillRectangle(*main_window->InnerWriter(), {20, 4}, {80, 16}, {0xc6, 0xc6, 0xc6});
